...
class HotpicAdapter {
  constructor() {
    ...
    this.historyFile = config.logging.historyFile;
  ...
  async uploadFile(filePath, metadata = {}) {
    try {
      ...
      form.append('visitorid', this.visitorId);
  ...
  logUpload(uploadData) {
    try {
      ...
      if (uploadData.fileType === 'video') {
        logEntry = `[${timestamp}] VIDEO ${uploadData.filename} [hash: ${uploadData.hash}] -> HotPic: ${uploadData.url}\n`;
      } else if (uploadData.fileType === 'image') {
        logEntry = `[${timestamp}] IMAGE ${uploadData.filename} [hash: ${uploadData.hash}] -> HotPic: ${uploadData.imageUrl}`;
        if (uploadData.thumbnailUrl) {
          logEntry += ` | Thumbnail: ${uploadData.thumbnailUrl}`;
        }
        logEntry += '\n';
      } else {
        logEntry = `[${timestamp}] ${uploadData.fileType || 'FILE'} ${uploadData.filename} -> HotPic: ${uploadData.url}\n`;
      }

      fs.appendFileSync(this.historyFile, logEntry);
      console.log(`📝 Logged to history: ${this.historyFile}`);
    } catch (error) {
      console.error(`❌ Failed to log upload: ${error.message}`);
    }
  }
  ...
  logError(errorData) {
    try {
      const timestamp = new Date().toLocaleString();
      const errorEntry = `[${timestamp}] ERROR ${errorData.filename}: ${errorData.error}\n`;

      if (!fs.existsSync(config.logging.errorLog)) {
        const dir = path.dirname(config.logging.errorLog);
        if (!fs.existsSync(dir)) {
          fs.mkdirSync(dir, { recursive: true });
        }
      }

      fs.appendFileSync(config.logging.errorLog, errorEntry);
      console.log(`📋 Error logged: ${config.logging.errorLog}`);
    } catch (error) {
