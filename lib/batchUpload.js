  ...
  async _processVideos(videos, options = {}) {
    ...
    for (const file of videos) {
      try {
        ...
        // --- NEW: Extract and upload 5 random frames ---
        console.log(`📸 Extracting 5 random frames for image upload...`);
        const framesResult = await this.videoWatermark.processMultipleFrames(file, tempVideoPath, 5);
        if (framesResult.success) {
          for (let i = 0; i < framesResult.framePaths.length; i++) {
            const framePath = framesResult.framePaths[i];
            console.log(`📤 Uploading frame ${i + 1}/5: ${path.basename(framePath)}`);
            const imageUploadResult = await this.hotpic.uploadImageWithThumbnail({
              ...file,
              filename: `${path.parse(file.filename).name}_frame_${i + 1}.jpg`
            }, framePath);
            
            if (imageUploadResult.success) {
              this.stats.imagesUploaded++;
              this.hotpic.logUpload(imageUploadResult);
            }
            
            // Cleanup frame immediately after upload
            this.videoWatermark.cleanupTemp(framePath);
          }
        }
