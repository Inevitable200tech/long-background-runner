...
class BatchUpload {
  ...
  async run(options = {}) {
    try {
      ...
      if (config.features.uploadVideos && filtered.videos.length > 0) {
        await this._processVideos(filtered.videos, options);
      }
   ...
   * @private
   */
  async _processVideos(videos, options = {}) {
    ...
    for (const file of videos) {
      try {
        ...
        // --- NEW: Extract 5 random frames for the gallery ---
        console.log(`📸 Extracting 5 random frames for the gallery...`);
        const framesResult = await this.videoWatermark.processMultipleFrames(file, tempVideoPath, 5);
        const framePaths = framesResult.success ? framesResult.framePaths : [];
        ...
        if (config.features.watermarkVideos) {
          ...
          const uploadsPath = path.join(uploadsDir, path.basename(watermarkResult.watermarkedPath));
          fs.renameSync(watermarkResult.watermarkedPath, uploadsPath);
          console.log(`📂 Watermarked video saved to: ${uploadsPath}`);

          // Upload video and gallery together in ONE album
          const uploadResult = await this.hotpic.uploadVideoWithGallery(file, uploadsPath, framePaths);

          if (uploadResult.success) {
            this.stats.videosUploaded++;
            this.stats.imagesUploaded += framePaths.length;
            if (uploadResult.url) this.stats.albums.push(uploadResult.url);
            this.hotpic.logUpload(uploadResult);
            this.filter.markAsUploaded(file.hash);
            console.log(`✅ Video and gallery uploaded successfully to one album`);
          ...
          const uploadResult = await this.hotpic.uploadVideoWithGallery(file, tempVideoPath, framePaths);
          if (uploadResult.success) {
            this.stats.videosUploaded++;
            this.stats.imagesUploaded += framePaths.length;
            if (uploadResult.url) this.stats.albums.push(uploadResult.url);
        ...
        // Cleanup all frames
        framePaths.forEach(fp => this.videoWatermark.cleanupTemp(fp));
        ...
        if (previewPath && fs.existsSync(previewPath)) {
          ...
          const finalPreviewPath = path.join(uploadsDir, path.basename(previewPath));
